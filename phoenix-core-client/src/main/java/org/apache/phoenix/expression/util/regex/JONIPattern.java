...
public class JONIPattern extends AbstractBasePattern implements AbstractBaseSplitter {
  ...
  public void replaceAll(ImmutableBytesWritable srcPtr, byte[] rStrBytes, int rStrOffset,
    ...
    byte[] replacedBytes = replaceAll(srcPtr.get(), srcPtr.getOffset(), srcPtr.getLength(),
      rStrBytes, rStrOffset, rStrLen);
  ...
  public void substr(ImmutableBytesWritable ptr, int offsetInStr) {
    ...
    int offsetInBytes = StringUtil.calculateUTF8Offset(ptr.get(), ptr.getOffset(), ptr.getLength(),
      SortOrder.ASC, offsetInStr);
    if (offsetInBytes < 0) {
