...
public final class PhoenixTracing {
  ...
  /**
   * Returns the global tracer for Phoenix. The tracer is obtained from {@link GlobalOpenTelemetry}
   * on each call (the OTel SDK caches it internally, so there is no performance penalty). This
   * avoids issues with eager initialization when the SDK has not been configured yet.
   */
  ...
  /**
   * Create a {@link SpanKind#CLIENT} span. Use this for outgoing RPC calls from Phoenix client to
   * HBase (e.g., scan requests, mutation commits).
   */
  ...
  public static Span createRemoteSpan(String name, Context ctx) {
    return getTracer().spanBuilder(name).setParent(ctx).setSpanKind(SpanKind.SERVER).startSpan();
  }
   ...
   * </pre>
   */
  public static <R, T extends Throwable> R trace(ThrowingCallable<R, T> callable, String spanName)
    throws T {
  ...
  /**
   * Wrap the provided {@link Runnable} in a traced {@link Runnable}, using a custom span supplier.
   */
  ...
  /**
   * Create a {@link CallWrapper} that wraps a call with an OpenTelemetry span. The span is started
   * in {@code before()} and ended in {@code after()}. This is used with
